...
namespace ai {
    namespace Scavenger {
        ...
        EnvironmentModel::EnvironmentModel() {
            ...
            mGrass                  = mTextureMgr->LoadTexture( "data/grass.tga", true );
            mMud                    = mTextureMgr->LoadTexture( "data/mud.tga", true );
            mRock                   = mTextureMgr->LoadTexture( "data/rock.tga", true );
            mCliff                  = mTextureMgr->LoadTexture( "data/cliff.tga", true );
            mIce                    = mTextureMgr->LoadTexture( "data/ice.tga", true );

            mRockTemplate->LoadModel( "data/rock.obj", mRock );
        }

        EnvironmentModel::~EnvironmentModel() {}

        void EnvironmentModel::LoadEnvironment( std::map<Location, Cell*> *cells ) {
            ...
            for( iCells = cells->begin(); iCells != cells->end(); ++iCells ) {
                if ( iCells->second == 0 ) {
                ...
                Vector3 tCenter             = { tLocation.GetX(), tLocation.GetY(), tLocation.GetElevation() };
                Vector3 tTopLeft            = { tLocation.GetX() + tXChange[3], tLocation.GetY() + tYChange[3], tLocation.GetElevation() };
                Vector3 tTopCenter          = { tLocation.GetX(), tLocation.GetY() + tYChange[0], tLocation.GetElevation() };
                Vector3 tTopRight           = { tLocation.GetX() + tXChange[0], tLocation.GetY() + tYChange[0], tLocation.GetElevation() };
                Vector3 tRightCenter        = { tLocation.GetX() + tXChange[0], tLocation.GetY(), tLocation.GetElevation() };
                Vector3 tBottomRight        = { tLocation.GetX() + tXChange[1], tLocation.GetY() + tYChange[1], tLocation.GetElevation() };
                Vector3 tBottomCenter       = { tLocation.GetX(), tLocation.GetY() + tYChange[1], tLocation.GetElevation() };
                Vector3 tBottomLeft         = { tLocation.GetX() + tXChange[2], tLocation.GetY() + tYChange[2], tLocation.GetElevation() };
                Vector3 tLeftCenter         = { tLocation.GetX() + tXChange[2], tLocation.GetY(), tLocation.GetElevation() };
                ...
                // Neighboring Cells
                Cell* tNorthNeighbor = iCells->second->GetNeighbor( Location::NORTH );
                Cell* tEastNeighbor  = iCells->second->GetNeighbor( Location::EAST );
                Cell* tSouthNeighbor = iCells->second->GetNeighbor( Location::SOUTH );
                Cell* tWestNeighbor  = iCells->second->GetNeighbor( Location::WEST );
                ...
                for ( int iCorner = 0; iCorner < 4; ++iCorner ) {
                    ...
                    if ( ( tCornerInterfaces[iCorner].first == "wall" || tCornerInterfaces[iCorner].first == "cliff" ) &&
                        ...
                        if ( tEdgeNeighbors[iCorner].second &&
                            ...
                            tCornerNeighbor = tEdgeNeighbors[iCorner].second->GetNeighbor( tDirections[iCorner].first );
                        }
                        if ( tEdgeNeighbors[iCorner].first &&
                             ( tCornerInterfaces[iCorner].first != "wall" && tCornerInterfaces[iCorner].first != "cliff" ) ) {
                            tZ += tEdgeNeighbors[iCorner].first->GetLocation().GetElevation();
                            tCount ++;
                            tCornerNeighbor = tEdgeNeighbors[iCorner].first->GetNeighbor( tDirections[iCorner].second );
                        }
                        if ( tCornerNeighbor ) {
        ...
        void EnvironmentModel::DrawModel( std::map< std::pair< float, float>, std::vector< bool >  > visited, 
            ...
            glColor4f( 1.0f, 1.0f, 1.0f, 1.0f );
            for ( int iObject = 0; iObject < mTerrainObjects.size(); ++iObject ) {
