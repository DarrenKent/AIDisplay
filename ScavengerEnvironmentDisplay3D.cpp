...
namespace ai {
    namespace Scavenger {
        ...
        EnvironmentDisplay3D::EnvironmentDisplay3D( ai::Agent::Environment *env, int widthIn, int heightIn )
            ...
            mWorld->LoadEnvironment( &(mEnvironment->cells) );
            ...
            GLuint tBaseTex         = mTextureMgr->LoadTexture( "data/base.tga", false );
            GLuint tAgentTex        = mTextureMgr->LoadTexture( "data/drone.tga", false );

            tBase->LoadModel( "data/base.obj", NULL );
            tBaseAccents->LoadModel( "data/baseAccents.obj", NULL );
            tAgentModel->LoadModel( "data/drone.obj", NULL );
            tAgentAccents->LoadModel( "data/droneaccents.obj", NULL );
            tAgentProp->LoadModel( "data/droneprop.obj", NULL );

            // Initialize Agents
            for( int iAgent = 0; iAgent < mEnvironment->agents.size(); ++iAgent ) {
                DebugMessage( "Initializing Agent: " + std::to_string( (long double)iAgent ), 1 );
                Agent* tStoredAgent                 = static_cast< Agent* >( mEnvironment->agents[iAgent] );
            ...
            // Load Textures
            mTopLeft        = mTextureMgr->LoadTexture( "data/topleft.tga", false );
            mBottomRight    = mTextureMgr->LoadTexture( "data/bottomright.tga", false );
            mBatteryFull    = mTextureMgr->LoadTexture( "data/batteryfull.tga", false );
            mBattery75      = mTextureMgr->LoadTexture( "data/battery75.tga", false );
            mBattery50      = mTextureMgr->LoadTexture( "data/battery50.tga", false );
            mBattery25      = mTextureMgr->LoadTexture( "data/battery25.tga", false );
            mHealthBar      = mTextureMgr->LoadTexture( "data/healthbar.tga", false );
            mHealth         = mTextureMgr->LoadTexture( "data/health.tga", false );
            mAgentLabel     = mTextureMgr->LoadTexture( "data/agentlabel.tga", false ); 
            mRecord         = mTextureMgr->LoadTexture( "data/record.tga", false );
        }
        ...
        EnvironmentDisplay3D::~EnvironmentDisplay3D() {
            ...
            for ( int iModel = 0; iModel < mModels.size(); ++iModel ) {
                if ( mModels[iModel] )
        ...
        void EnvironmentDisplay3D::SetEnv( ai::Agent::Environment *env_in ) {
            ...
            mMutex.lock();
                ...
                std::chrono::system_clock::time_point tNow = Clock::now();
                ...
                for( int iAgent = 0; iAgent < tEnv->agents.size(); ++iAgent ) {
                    Agent* tStoredAgent = static_cast< Agent* >( tEnv->agents[iAgent] );
                    ...
                    mAgentsInfo[iAgent].baseSize        = tStoredAgent->GetBase()->GetHopper().size();
        ...
        void EnvironmentDisplay3D::Redraw() {
            ...
            for ( int iAgent = 0; iAgent < mAgents.size(); ++iAgent )
        ...
        void EnvironmentDisplay3D::DrawScene() {
            ...
            if ( !mVisited )
                ...
                tAgentVisit = -1;
            mWorld->DrawModel( mVisitedCells, AGENT_COLORS, tAgentVisit, mGrid );
            glEnable( GL_LIGHTING );
            for ( int iModel = 0; iModel < mModels.size(); ++iModel )
        ...
        void EnvironmentDisplay3D::DrawGUI() {
            ...
            Action* tLastAction;
            mMutex.lock();
                tAgentX         = mAgentsInfo[mCurrentAgent].x;
                tAgentY         = mAgentsInfo[mCurrentAgent].y;
                tAgentZ         = mAgentsInfo[mCurrentAgent].z;
            ...
            else if ( tLastAction && tLastAction->GetCode() >= 0 && tLastAction->GetCode() < mActionStrings.size() )
        ...
        void EnvironmentDisplay3D::ProcessInput() {
            ...
            if ( GetKeyPressed('q') && !GetKeyPressed('e') ) {
                mCurrentAgent ++;
                if ( mCurrentAgent >= mAgentsInfo.size() )
                    mCurrentAgent = 0;
            }
